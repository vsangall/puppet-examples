...
**TLDR**: This module manages HAProxy load balancer installations with support for multiple backends, SSL termination, and statistics reporting. It handles package installation, configuration management, service lifecycle, and firewall rules. The module uses a hierarchical configuration approach with deep merging of backend configurations across different hierarchy levels.
...
- **HAProxy**: HTTP/HTTPS load balancer
  ...
  - Key Config: /etc/haproxy/haproxy.cfg, /etc/haproxy/conf.d/*.cfg
...
1. **profile_haproxy** (`manifests/init.pp`):
   - Sets class parameters from Hiera: package_name=haproxy, config_dir=/etc/haproxy, config_file=/etc/haproxy/haproxy.cfg, service_name=haproxy, user=haproxy, group=haproxy, stats_enabled=true/false (environment-dependent), stats_port=9000/9001 (node-dependent), stats_uri=/haproxy-stats, stats_user=admin, stats_password=(encrypted), global_maxconn=4096/16384/32768 (varies by environment/cluster), client_timeout=30s/60s (environment-dependent), server_timeout=30s/60s (environment-dependent), connect_timeout=5s, retries=3, ssl_enabled=false/true (environment-dependent), ssl_cert_path=/etc/ssl/certs, ssl_key_path=/etc/ssl/private, ssl_ciphers=(varies by environment/cluster), ssl_min_version=TLSv1.2/TLSv1.3 (cluster-dependent), log_server=127.0.0.1/10.100.1.50 (datacenter-dependent), log_facility=local0, log_level=info/warning/debug (environment-dependent), backends=(hash of backend configurations)
   - Contains profile_haproxy::install class
   - Contains profile_haproxy::config class
   - Contains profile_haproxy::service class
   - Contains profile_haproxy::firewall class
...
3. **profile_haproxy::config** (`manifests/config.pp`):
   ...
   - Iterations: Runs 3 times for backends defined in Hiera ($backends hash): webservers, api, internal_monitoring (cluster-specific)
     ...
     - **api**:
       - Backend servers: api1-fra (10.100.2.10:3000, weight=200 on lb01), api2-fra (10.100.2.11:3000, weight=100)
     ...
     - **internal_monitoring** (cluster-specific):
   ...
   - Conditional: If stick_table_enabled=true (production environment):
     - Creates stick-table config: /etc/haproxy/conf.d/stick-tables.cfg (mode: 0640)
...
4. **profile_haproxy::service** (`manifests/service.pp`):
   ...
   - Manages service: haproxy (ensure=running, enable=true, hasrestart=true, hasstatus=true)
   - Deploys logrotate config: /etc/logrotate.d/haproxy (mode: 0644)
...
5. **profile_haproxy::firewall** (`manifests/firewall.pp`):
   - Conditional: If firewall_provider=ufw (Debian OS):
     - Installs package: ufw
...
**Variable Flow Summary**: 26 variables across 7 Hiera levels with deep merging for backend configurations
...
**nodes/lb01.fra.example.com.yaml (node-specific overrides)** → Migration note: Node-specific variables, loaded based on node certname
- `profile_haproxy::stats_enabled`: `true` (type: boolean)
...
- Variables using `deep` merge - The backends hash is deeply merged across hierarchy levels
...
- **Purpose**: Detects the installed HAProxy version on Linux systems
- **Implementation**: Executes 'haproxy -v' command and extracts version number using regex
- **Platform constraints**: Linux only (confine kernel: 'Linux')
- **Migration notes**: This fact provides the installed HAProxy version for conditional logic. In Ansible, this can be implemented as a fact using the command module with a similar regex to extract the version.
...
- **Complex expressions**: None, mostly simple variable substitutions
...
- **Complex expressions**: None, mostly simple variable substitutions and conditionals
...
# HTTPS endpoint check (if SSL enabled)
curl -I https://localhost

# Stats page check (if stats enabled)
curl http://localhost:9000/haproxy-stats
# or
curl http://localhost:9001/haproxy-stats

# Firewall status check (on Debian)
ufw status

# Backend connectivity checks
curl -I http://10.100.1.10:8080/health
curl -I http://10.100.2.10:3000/api/health
curl -I http://10.100.3.10:9090/-/healthy
```