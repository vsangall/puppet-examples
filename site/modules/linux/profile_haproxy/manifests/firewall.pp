...
# Uses base_utils::normalize_port() for cross-module function call example.
class profile_haproxy::firewall {

  $firewall_provider = lookup('profile_haproxy::firewall_provider', String, 'first', 'ufw')

  # Cross-module function call: normalize stats port to Integer
  $normalized_stats_port = base_utils::normalize_port($profile_haproxy::stats_port)
